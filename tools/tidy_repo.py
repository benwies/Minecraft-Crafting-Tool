...
def compact_import_newlines(text: str) -> str:
    ...
    # Ensure only a single blank line after the import block
    i = 0
...
def collapse_blanks_and_trailing(text: str) -> str:
    ...
    for ln in text.split('\n'):
        # Replace leading tabs with 4 spaces to avoid mixed-indentation issues
    ...
    text = re.sub(r'\n{3,}', '\n\n', text)  # first reduce 3+ to 2
    text = re.sub(r'\n{2,}', '\n\n', text)  # then reduce any 2+ to exactly 1 blank line
    return text

def tidy_code(text: str) -> str:
    a = compact_import_newlines(text)
    b = collapse_blanks_and_trailing(a)
    # Ensure single trailing newline if original had one
    if not b.endswith('\n'):
        b += '\n'
    return b
